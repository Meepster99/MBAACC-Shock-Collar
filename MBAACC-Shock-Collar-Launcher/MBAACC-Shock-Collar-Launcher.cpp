...
std::wstring getLogPath() {
	...
	std::wstring res = std::wstring(buffer) + std::wstring(L"\\MBAACC-Shock-Collar-Launcher.log");
...
std::wstring getExePath() {
	...
	std::wstring dllPath = std::wstring(buffer) + std::wstring(L"\\MBAACC-Shock-Collar.exe");
	return dllPath;
}
...
int needUpdate() {
	...
	HINTERNET hConnect = InternetOpenUrlW(hInternet, L"https://api.github.com/repos/Meepster99/MBAACC-Shock-Collar/releases/tags/bleeding-edge", NULL, 0, INTERNET_FLAG_RELOAD, 0);
	if (hConnect == NULL) {
...
bool update() {
	...
	// https://github.com/fangdreth/MBAACC-Extended-Training-Mode/releases/download/bleeding-edge/MBAACC-Extended-Training-Mode-Launcher.exe
	// https://github.com/Meepster99/MBAACC-Shock-Collar/releases/download/bleeding-edge/MBAACC-Shock-Collar-Launcher.exe
	const wchar_t* urlString = L"https://github.com/Meepster99/MBAACC-Shock-Collar/releases/download/bleeding-edge/MBAACC-Shock-Collar-Launcher.exe";
	HINTERNET hConnect = InternetOpenUrlW(hInternet, urlString, NULL, 0, INTERNET_FLAG_RELOAD, 0);
	if (hConnect == NULL) {
		log("InternetOpenUrlW failed: %d", GetLastError());
		InternetCloseHandle(hInternet);
		return false;
